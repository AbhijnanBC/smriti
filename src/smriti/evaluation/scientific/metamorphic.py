...
from smriti.scoring.policies import EVIDENCE_SIGNAL_NAMES, load_policy
...
def _make_node(
    ...
    topo = TopologyMetrics(
        degree=degree,
        in_degree=in_degree,
        out_degree=max(0, degree - in_degree),
        is_bridge=False,
        is_hub=False,
        partition_id="p0",
        centrality=0.3,
...
def _score(claim_id: str, graph: KnowledgeGraph):
    ...
    RECTIFIED (P1-4): reliability_index in production (scoring/__init__.py)
    is now fused from the evidence-family ContributionSet only (see
    scoring.policies.EVIDENCE_SIGNAL_NAMES) -- every MR below (independent
    support, contradiction/conflict, source duplication, determinism) is
    itself an evidence-family perturbation, so this mirrors production
    exactly rather than re-including the now-separate graph-importance
    signals (topology_strength/hub_score/bridge_score) that production no
    longer folds into this number.
    """
    policy = load_policy()
    ...
    evidence_cs = split_contribution_set_by_category(contribution_set, EVIDENCE_SIGNAL_NAMES)
    ri, uncertainty, _, _ = compute_reliability(
        evidence_cs,
        policy,
        signal_vector,
...
def run_reliability_metamorphic_suite(api=None) -> dict:
    ...
    perturbed_nodes = {
        "target": _make_node(
            "target", "doc_target", support_ids=("supporter_1",), degree=1, in_degree=1
        ),
    ...
    contested_nodes = {
        "target": _make_node(
            "target", "doc_target", support_ids=("supporter_1",), degree=2, in_degree=1
        ),
        "supporter_1": _make_node("supporter_1", "doc_other_1"),
    ...
    contested_edges = {
        "e1": _make_edge(
            "e1", "target", "opponent_1", RelationshipType.CONTRADICTS, confidence=0.9
        ),
    }
    ...
    results.append(
        {
            ...
            "passed": (ri_mr2 <= ri_mr1)
            and (sv_mr2.conflict_pressure > 0.0)
            and (unc_mr2 > unc_mr1),
            "before": {
                "reliability": round(ri_mr1, 3),
                "conflict_pressure": 0.0,
                "uncertainty": round(unc_mr1, 3),
            },
            "after": {
                "reliability": round(ri_mr2, 3),
                "conflict_pressure": round(sv_mr2.conflict_pressure, 3),
                "uncertainty": round(unc_mr2, 3),
            },
        }
    )
    ...
    duplicate_source_nodes = {
        "target": _make_node(
            "target",
            "doc_target",
            support_ids=("supporter_1", "supporter_2"),
            degree=2,
            in_degree=2,
        ),
        "supporter_1": _make_node("supporter_1", "doc_other_1"),
