...
_PERMITTED_TRANSITIONS: dict[RuntimeState, frozenset[RuntimeState]] = {
    ...
    RuntimeState.RECOVERING: frozenset(
        {RuntimeState.ACTIVE, RuntimeState.DEGRADED, RuntimeState.TERMINATING}
    ),
...
class RuntimeStateMachine:
        ...
        publish(
            (
                ...
                if target == RuntimeState.ACTIVE
                    ...
                    ArchitectureEventType.SHUTDOWN_INITIATED
                    if target == RuntimeState.TERMINATING
                    else (
                        ArchitectureEventType.RECOVERY_STARTED
    ...
    def register_observer(
        self, callback: Callable[[RuntimeState, RuntimeState, str], None]
    ) -> None:
    ...
    def current_state_entry_time(self) -> float | None:
        if self._history:
