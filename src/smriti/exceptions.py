...
class NormalizationError(ParsingError):
    ...
    RECTIFIED (publication-readiness audit): this name was independently
    redefined three more times further down this file (as a Phase3Error,
    a Phase8Error, and a Phase9Error) with none of the later three ever
    actually raised anywhere in the codebase -- each one silently shadowed
    the previous `NormalizationError` binding at module-load time, so
    `smriti.parsing.normalize.py`'s own `from smriti.exceptions import
    NormalizationError` (this class's real, only caller) was actually
    resolving to the Phase9 one, not this one. The three unused
    duplicates were removed rather than renamed, since nothing referenced
    them; a future phase that needs its own normalization-failure
    exception should give it a phase-specific name (e.g.
    SignalNormalizationError, QueryNormalizationError) instead of reusing
    this one.
    """
...
class NLIEvaluationInferenceFailureError(Phase6Error):
    """
    RECTIFIED (P1-I, "FINAL REVIEW" round): raised when one or more NLI
    batches failed during an evaluation run (config env == "eval") and
    evaluation.allow_nli_inference_failures is not explicitly set true.
    In production, a failed batch is still skipped and logged (see
    NLIInferenceBatchError) -- silently dropping a few pairs under
    genuine transient failure is an acceptable production degradation.
    In evaluation, the same silent drop would let "1000 attempted, 100
    failed, 900 resolved" masquerade as "900 successfully evaluated" in
    every downstream metric, so evaluation runs fail loudly by default
    instead.
    """
...
class IndexRegistryError(Phase9Error):
    """
    Index registry or builder encountered an error.

    RECTIFIED (publication-readiness audit): renamed from `IndexError`,
    which shadowed the Python builtin of the same name for any module
    importing this one. Never actually raised anywhere in the codebase
    under either name.
    """
