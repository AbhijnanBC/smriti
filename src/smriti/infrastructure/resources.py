...
RESOURCE_POLICIES: Dict[str, ResourcePolicy] = {
    "claim_view_cache": ResourcePolicy(
        kind=ResourceKind.CACHE, owner="api.store.memory_store",
        max_size_bytes=50 * 1024 * 1024, ttl_seconds=None, eviction_strategy="lru",
    ),
    "dto_cache": ResourcePolicy(
        kind=ResourceKind.CACHE, owner="api.dtos",
        max_size_bytes=10 * 1024 * 1024, ttl_seconds=None, eviction_strategy="lru",
    ),
    "faiss_index": ResourcePolicy(
        kind=ResourceKind.INDEX, owner="retrieval.faiss_index",
        max_size_bytes=None, ttl_seconds=None, eviction_strategy="none",
        is_disposable=True, release_on_shutdown=True,
    ),
    "export_buffer": ResourcePolicy(
        kind=ResourceKind.EXPORT_BUFFER, owner="dashboard.export",
        max_size_bytes=25 * 1024 * 1024, ttl_seconds=300.0, eviction_strategy="ttl",
    ),
}
        ...
        with self._lock:
            ...
            if current + amount > budget.limit:
                if budget.enforcement == "block":
                    logger.error("budget_exceeded_blocked", budget=budget_name,
                                 current=current, proposed=amount, limit=budget.limit)
                    return False
                elif budget.enforcement == "warn":
                    logger.warning("budget_exceeded_warn", budget=budget_name,
                                   current=current, proposed=amount, limit=budget.limit)
                else:
...
class ResourceGovernor:
    ...
    def allocate(self, name: str, size_bytes: int = 0) -> ResourceHandle:
        policy = self._policies.get(name) or ResourcePolicy(
            kind=ResourceKind.CACHE, owner="unknown",
            max_size_bytes=None, ttl_seconds=None, eviction_strategy="none",
        )
        ...
        publish(ArchitectureEventType.RESOURCE_ALLOCATED, source="infrastructure.resources",
    ...
    def release(self, name: str) -> None:
        ...
        if handle and not handle.released:
            ...
            from smriti.runtime.events import publish, ArchitectureEventType
            publish(ArchitectureEventType.RESOURCE_RELEASED, source="infrastructure.resources", name=name)
