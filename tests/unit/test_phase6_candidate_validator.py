...
from smriti.retrieval.validator import RejectionReason, validate_candidates
...
def make_embedded_claim(claim_id: str, finite: bool = True, dim_ok: bool = True):
    vec = Vector(
        values=(0.1, 0.2, 0.3, 0.4), dimension=4, dtype=VectorDType.FLOAT64, normalized=True
    )
    ...
    quality = EmbeddingQuality(
        dimension_ok=dim_ok, normalized=True, finite=finite, cache_used=False
    )
...
def test_self_comparison_rejected():
    ...
    pair = CandidatePair(
        claim_id_a="c001", claim_id_b="c001", cosine_similarity=1.0, candidate_rank=0
    )
