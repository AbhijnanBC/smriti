...
def test_constructs_nodes_for_all_claims():
    ...
    result = run_construction(
        make_rel_set(rels), make_claims_map("c001", "c002"), NetworkXBackend()
    )
...
def test_unknown_relationships_filtered():
    ...
    result = run_construction(
        make_rel_set(rels), make_claims_map("c001", "c002", "c003"), NetworkXBackend()
    )
...
def test_neutral_included_when_configured():
    rels = [make_rel("r1", "c001", "c002", RelationshipType.NEUTRAL)]
    result = run_construction(
        make_rel_set(rels), make_claims_map("c001", "c002"), NetworkXBackend(), include_neutral=True
    )
    assert "r1" in result.edges
