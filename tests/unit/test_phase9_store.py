    ...
    def make_reliability(claim_id, ri, label):
        ...
        return ReliabilityMetadata(
            ...
            audit=ReliabilityAudit(
                ...
                graph_fingerprint="test_fingerprint",
                graph_schema_version="7.0",
                fusion_algorithm="v2",
                normalization_version="1.1",
                computed_at_run_id="r1",
...
def test_scan_returns_all_without_predicates(store):
    records, total = store.scan(
        predicates=[], sort=SortSpec("claim_id"), pagination=Pagination(limit=100)
    )
    assert total == 2
...
def test_scan_pagination(store):
    records, total = store.scan(
        predicates=[], sort=SortSpec("claim_id"), pagination=Pagination(limit=1, offset=0)
    )
...
def test_read_store_has_no_traverse_graph_method():
    ...
    assert not hasattr(ReadStore, "traverse_graph"), (
        "traverse_graph must NOT be in ReadStore. " "Traversal belongs in NavigationService."
    )
