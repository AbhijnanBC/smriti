...
def test_metamorphic_suite_runs_all_four_relations():
    ...
    assert (
        "MR2_contradiction_does_not_increase_reliability_and_raises_conflict_and_uncertainty"
        in relation_names
    )
...
def test_metamorphic_mr1_independent_support_raises_reliability_and_lowers_uncertainty():
    ...
    mr1 = next(
        ...
        if r["relation"] == "MR1_independent_support_increases_reliability"
    )
...
def test_metamorphic_mr3_source_diversity_is_identical_not_just_non_increasing():
    ...
    mr3 = next(
        r
        for r in result["relations"]
        if r["relation"] == "MR3_same_document_duplicate_does_not_raise_source_diversity"
    )
...
def test_reconstruction_audit_reconstructs_a_real_scored_claim():
    ...
    graph = _make_graph(
        {
            "target": _make_node(
                "target", "doc_target", support_ids=("s1",), degree=1, in_degree=1
            ),
    ...
    contribution_set, _, sv = assemble_contribution_set(
        raw_signals, extractors, policy.fusion, "target"
    )
    ...
    record = {
        ...
        "component_scores": [{"contribution": c.contribution} for c in component_scores],
    }
