...
SMRITI-Reference's real prose, CONTRADICTS precision is 4.5\% and recall
...
rather than direct propositional negation, and a model trained on
...
\paragraph{No probability calibration is actually fitted.} Phase 6
supports temperature, percentile, and isotonic calibration strategies
for the NLI model's raw confidence
(\texttt{retrieval/classification/calibration.py}), but
\texttt{config/default.yaml} leaves every model's calibration entry
empty, so every run in this paper uses the identity strategy (raw
confidence passed through unchanged). We therefore do not call the NLI
scores used throughout this paper ``calibrated'' in the technical sense
(probabilities matching empirical frequencies, measurable via ECE or a
reliability diagram) --- only that the underlying model was itself
trained on SNLI/MultiNLI/ANLI-style data. Fitting and evaluating one of
the implemented strategies against a held-out sample is concrete,
already-half-built future work.
