...
no detectable vocabulary; (iii) we built SMRITI-Controlled, a 713-pair, 15-category
corpus of claim pairs whose relationship label is a fact about how the
corpus was constructed (numeric facts, explicit negation, named-entity
attribution), requiring no annotation of any kind, on which the fixed
classifier achieves 98.3\% classification precision across 119 frozen-TEST
retrieved contradiction pairs spanning six distinct contradiction categories; (iv) we added metamorphic reliability tests and
...
argumentative prose: CONTRADICTS precision there is 4.5\%, with the
pre-fix figure (3.2\%) falling inside this measurement's own 95\%
bootstrap CI, i.e.\ no evidence of improvement, because most of that
corpus's genuine disagreement is rhetorical rather than propositional,
and an off-the-shelf entailment model does not recognize rhetoric as
contradiction regardless of how faithfully the resolver reports its
verdict. A second adversarial-audit round then identified a deeper
architectural defect in the same relationship classifier: NLI ran in only
one fixed direction per candidate pair, an artifact of the pair's
claim-ID lexicographic ordering rather than a semantic decision, making
it structurally impossible to distinguish ``A entails B'' from ``B
entails A'' from genuine equivalence, and hardcoding every SUPPORTS
edge's reported direction regardless of which claim actually did the
confirming. We rebuilt Phase 6 to run NLI bidirectionally, added
EQUIVALENT as a fifth relation type for claims that entail each other in
both directions, and derived SUPPORTS/REFINES direction genuinely from
NLI rather than ID order; scaled SMRITI-Controlled from 32 hand-written
pairs to 713 pairs across 15 categories so its findings would be rates
rather than anecdotes (revealing that a same-entity multi-attribute
confusion, reported at pilot scale as a single anecdote, is in fact this
evaluation's single largest source of classification error; and that
our own initial SUPPORTS measurement on this same corpus was itself
wrong and needed correction against the frozen results artifact --- the
corrected number shows SUPPORTS is strong on this corpus once retrieved,
not a systematic weakness); and
re-ran the SMRITI-Reference annotation a second time, which found
real-prose CONTRADICTS precision (14.3\%) sits within a 95\% bootstrap CI
wide enough to contain both prior measurements, so we make no claim of a
distinguishable change from a single fresh sample with this few positive
calls, and caught a genuine hardcoded-direction defect the bidirectional
rewrite itself introduced in one resolver fallback branch (0/9 direction
accuracy pre-fix on the annotated subset; 44.4\% after fixing it, still
roughly chance-level). A further audit round then found
that fallback branch was itself semantically wrong regardless of its
hardcoded direction, and a second, unrelated bug in the specificity gate
that separates SUPPORTS from REFINES; fixing both collapses REFINES
recall on this same real-corpus sample to 2.3\% (from an earlier,
heuristic-inflated 26.5\% F1) and direction availability to 2.2\% (from
20\%) --- a less flattering picture than the 44.4\% figure above, and one
we report specifically because it is less flattering, not despite it. We
report all of this, including what still does not work and what our own
rewrites broke along the way, with the same discipline we applied to
finding what was broken the first time.
\end{abstract}
...
\begin{itemize}
  ...
  \item \textbf{SMRITI-Controlled}, a 713-pair, 15-category corpus with an
    ...
    categories, SUPPORTS, REFINES, EQUIVALENT, four hard-NEUTRAL subtypes,
    an easy-NEUTRAL control, and a deliberately-ambiguous abstention check,
    that separately measures retrieval recall and classification accuracy
    per the review's recommendation to distinguish the two
    (\S\ref{sec:controlled}).
  \item \textbf{A bidirectional-NLI rewrite of relationship discovery}:
    Phase 6 previously ran NLI in one direction per pair, fixed by claim-ID
    lexicographic order rather than semantics, making SUPPORTS's reported
    direction a hardcoded artifact and equivalence undetectable in
    principle. NLI now runs both directions per pair (one batched call,
    not two), EQUIVALENT is added as a fifth relation type for pairs that
    entail each other both ways, and SUPPORTS/REFINES direction is derived
    genuinely from NLI. This same rewrite's own annotation round caught
    and let us fix a hardcoded-direction defect it introduced in one
    resolver fallback branch (\S\ref{sec:system}, \S\ref{sec:bugs}).
  ...
  \item A disclosed, re-run LLM-derived reference annotation
    ...
    yielding 4.5\% real-corpus CONTRADICTS precision alongside
    SMRITI-Controlled's 98.0\%, and a full accounting of what remains
...
evidence as a standard task, and SciFact \citep{wadden2020fact} extended
it to scientific claims verified against research abstracts, later
extended to the open-domain setting by SciFact-Open
\citep{wadden2022scifact}; \S\ref{sec:fever}, \S\ref{sec:scifact}, and
\S\ref{sec:scifact-open} evaluate SMRITI's own resolver zero-shot against
all three. NLI datasets such as SNLI
...
resolver that never checked the third class. \citet{tan-etal-2026-scitrue}'s
concurrent SciTrue targets source-level accountability and evidence
traceability directly, with a genuine human evaluation (300 attributions)
this paper does not have (\S\ref{sec:limitations}) --- we do not claim
priority on evidence traceability as a research problem, only that
SMRITI's own provenance/relationship model (\S\ref{sec:system}) is one
additional, differently-motivated approach to a substantially related
concern (contradiction-aware knowledge organization rather than
attribution-focused claim verification), evaluated throughout this paper
by construction-defined and audit-round methods rather than by human
judgment.
...
\input{sections/annotation_protocol.tex}

\input{sections/controlled_corpus.tex}

\input{sections/metamorphic.tex}
